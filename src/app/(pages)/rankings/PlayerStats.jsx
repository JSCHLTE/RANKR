  ...
  return (
    <>
          ...
          <tr>
            ...
            <th className='border-left'>{labels.team.label}</th>
    ...
    return (
      <>
          ...
          <>
            ...
            <td><div className={getStat(item.totals?.targets, seasonTotals.WR.targets.low, seasonTotals.WR.targets.mid)}>{item.totals?.targets ?? 0}</div></td>
            ...
            <td><div className={getStat(item.totals?.rushingAttempts, seasonTotals.WR.rushingAttempts.low, seasonTotals.WR.rushingAttempts.mid)}>{item.totals?.rushingAttempts ?? 0}</div></td>
            ...
            <td className='border-right'><div className={getStatLow(item.totals?.fumbles, seasonTotals.WR.fumbles.low, seasonTotals.WR.fumbles.mid)}>{item.totals?.fumbles ?? 0}</div></td>
            </>
          ...
          { player.position === "TE" ? 
            ...
            <td><div className={getStat(item.totals?.fantasyPoints, seasonTotals.TE.fantasyPoints.low, seasonTotals.TE.fantasyPoints.mid)}>{
            item.totals?.gamesPlayed
              ? (item.totals.fantasyPoints / item.totals.gamesPlayed).toFixed(1)
              : "—"
            }</div></td>
            ...
            <td><div className={getStat(item.totals?.targets, seasonTotals.TE.targets.low, seasonTotals.TE.targets.mid)}>{item.totals?.targets ?? 0}</div></td>
            ...
            <td><div className={getStatLow(item.totals?.fumbles, seasonTotals.TE.fumbles.low, seasonTotals.TE.fumbles.mid)}>{item.totals?.fumbles ?? 0}</div></td>
            </>
          ...
          { player.position === "RB" ? 
            ...
            <td><div className={`table-stat ${item.totals?.gamesPlayed <= 14 ? "red" : "green"}`}>{item.totals?.gamesPlayed ?? "—"}</div></td>
              ...
              <td><div className={`${getStat(item.totals?.fantasyPoints, seasonTotals.RB.fantasyPoints.low, seasonTotals.RB.fantasyPoints.mid)}`}>{
                item.totals?.gamesPlayed
                ? (item.totals.fantasyPoints / item.totals.gamesPlayed).toFixed(1)
                : "—"
              }</div></td>
              ...
              <td><div className={getStatLow(item.totals?.fumbles, seasonTotals.RB.fumbles.low, seasonTotals.RB.fumbles.mid)}>{item.totals?.fumbles ?? 0}</div></td>
              <td><div className={getStat(item.totals?.targets, seasonTotals.RB.targets.low, seasonTotals.RB.targets.mid)}>{item.totals?.targets ?? 0}</div></td>
          ...
          { player.position === "QB" ? 
            ...
            <td><div className={getStat(item.totals?.fantasyPoints, seasonTotals.QB.fantasyPoints.low, seasonTotals.QB.fantasyPoints.mid)}>{
            item.totals?.gamesPlayed
              ? (item.totals.fantasyPoints / item.totals.gamesPlayed).toFixed(1)
              : "—"
            }</div></td>
              <td><div className={getStat(item.totals?.passingAttempts, seasonTotals.QB.passingAttempts.low, seasonTotals.QB.passingAttempts.mid)}>{item.totals?.passingAttempts ?? 0}</div></td>
              ...
              <td><div className={getStatLow(item.totals?.interceptions, seasonTotals.QB.interceptions.low, seasonTotals.QB.interceptions.mid)}>{item.totals?.interceptions ?? 0}</div></td>
              <td><div className={getStat(item.totals?.rushingAttempts, seasonTotals.QB.rushingAttempts.low, seasonTotals.QB.rushingAttempts.mid)}>{item.totals?.rushingAttempts ?? 0}</div></td>
